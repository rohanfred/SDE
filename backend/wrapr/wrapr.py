...
load_dotenv()
engine=create_engine(os.getenv('DATABASE_URL'),pool_size=0,pool_pre_ping=True)
conn=engine.connect()
...
for apinm,reqs in [
    ('register',get_signups(nusr)),
    ('login',get_signins(ssnid,nusr)),
    ('upload',get_uploads(ssnid,nusr)),
    ('search',get_searches(ssnid,nusr))
    ]:
    # print(f'\n>>>({apinm}) requests:',reqs)
    results=main(reqs,typ="post")
    print(f'\n>>>({apinm}) results:',results)

print(f'\n======Total time taken======{(perf_counter()-st):.2f} seconds')